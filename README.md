...
This project analyzes the [HR department dataset](https://www.kaggle.com/datasets/mfaisalqureshi/hr-analytics-and-job-prediction) to build predictive models that estimate the likelihood of employee attrition. The workflow includes data cleaning, exploratory analysis, and visualization to uncover patterns related to turnover. Three models—**Logistic Regression**, **Decision Tree** and **Random Forest**—were developed and evaluated based on metrics such as accuracy, precision, recall, and F1 score. To recommend the most effective model, the project balances both predictive performance and interpretability. Additionally, feature importance analysis was performed to highlight key factors driving employee departures, offering valuable insights for HR strategies.
